import Link from "next/link";
import { FaInstagram, FaFacebook, FaPhone, FaMapMarkerAlt, FaClock } from "react-icons/fa";
import { MdEmail } from "react-icons/md";

export default function Footer() {
  return (
    <footer className="bg-gradient-to-r from-[#ffb8d5] via-[#ffd6e7] to-[#ffe8f0] text-gray-800 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        {/* Salon Heading */}
        <h2 className="text-3xl md:text-4xl font-bold text-center mb-8 md:mb-12 text-gray-800">
          Salon
        </h2>

        {/* Contact Info and Opening Hours - Side by Side */}
        <div className="flex flex-col md:flex-row gap-8 md:gap-12 justify-center mb-8">
          {/* Contact Info */}
          <div className="bg-white/60 backdrop-blur-sm p-6 md:p-8 rounded-xl shadow-lg flex-1 max-w-md">
            <h3 className="text-xl md:text-2xl font-bold mb-4 md:mb-6 text-pink-600 text-center">
              Contact Us
            </h3>
            <div className="space-y-4">
              <div className="flex items-start space-x-3">
                <FaMapMarkerAlt className="text-pink-600 text-xl mt-1 flex-shrink-0" />
                <span>123 Beauty Street, Salon City</span>
              </div>
              <div className="flex items-center space-x-3">
                <FaPhone className="text-pink-600 text-xl" />
                <span>(123) 456-7890</span>
              </div>
              <div className="flex items-center space-x-3">
                <MdEmail className="text-pink-600 text-xl" />
                <span>contact@salon.com</span>
              </div>
              <div className="flex justify-center space-x-6 pt-2">
                <a href="https://instagram.com" target="_blank" rel="noopener noreferrer" className="text-pink-600 hover:text-pink-800 transition-colors">
                  <FaInstagram className="text-2xl" />
                </a>
                <a href="https://facebook.com" target="_blank" rel="noopener noreferrer" className="text-pink-600 hover:text-pink-800 transition-colors">
                  <FaFacebook className="text-2xl" />
                </a>
              </div>
            </div>
          </div>

          {/* Opening Hours */}
          <div className="bg-white/60 backdrop-blur-sm p-6 md:p-8 rounded-xl shadow-lg flex-1 max-w-md">
            <h3 className="text-xl md:text-2xl font-bold mb-4 md:mb-6 text-pink-600 text-center">
              Opening Hours
            </h3>
            <div className="flex space-x-3">
              <FaClock className="text-pink-600 text-xl mt-1 flex-shrink-0" />
              <div className="space-y-2">
                <p className="whitespace-nowrap">Monday - Friday: 9am - 7pm</p>
                <p className="whitespace-nowrap">Saturday: 10am - 6pm</p>
                <p className="whitespace-nowrap">Sunday: Closed</p>
              </div>
            </div>
          </div>
        </div>

        {/* Quick Links - Below */}
        <div className="bg-white/60 backdrop-blur-sm p-6 md:p-8 rounded-xl shadow-lg max-w-2xl mx-auto">
          <h3 className="text-xl md:text-2xl font-bold mb-4 md:mb-6 text-pink-600 text-center">
            Quick Links
          </h3>
          <nav className="flex flex-wrap justify-center gap-4 md:gap-6">
            <Link href="/" className="hover:text-pink-600 transition-colors px-2 py-1">Home</Link>
            <Link href="/our-services" className="hover:text-pink-600 transition-colors px-2 py-1">Services</Link>
            <Link href="/about-us" className="hover:text-pink-600 transition-colors px-2 py-1">About Us</Link>
            <Link href="/contact-us" className="hover:text-pink-600 transition-colors px-2 py-1">Contact</Link>
            <Link href="/booking" className="hover:text-pink-600 transition-colors px-2 py-1">Book Appointment</Link>
          </nav>
        </div>

        {/* Copyright */}
        <div className="max-w-7xl mx-auto pt-8 mt-8 border-t border-pink-200 text-center">
          <p>&copy; {new Date().getFullYear()} Salon. All rights reserved.</p>
          <p className="text-sm">Designed by Haris Shabbir</p>
        </div>
      </div>
    </footer>
  );
}